...
def get_data(filters):
    ...
    for row in raw_data:
        submission_date = getdate(row.submission_dt) if row.submission_dt else None
        reference_date = getdate(row.reference_date) if row.reference_date else None

        if submission_date:
            if reference_date:
                # Paid → freeze aging at reference date
                aging = date_diff(reference_date, submission_date)
            else:
                # Not paid → running aging
                aging = date_diff(current_date, submission_date)

            # Safety: avoid negative values
            row.aging_days = max(aging, 0)
        else:
            row.aging_days = 0
