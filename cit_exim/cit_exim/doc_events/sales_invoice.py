...
#     # Example: E-062-25-26 splits into ['E', '062', '25', '26']
...
#         # 1. Construct the New Name
#         suffix = suffixes[i]
#         modified_parts = list(name_parts)
#         if len(modified_parts) >= 2:
#             # Changes '062' to '062A'
#             modified_parts[1] = f"{modified_parts[1]}{suffix}"
#         else:
#             modified_parts[0] = f"{modified_parts[0]}{suffix}"
            
#         target_name = "-".join(modified_parts)

#         # 2. Copy Parent Fields
#         exclude_fields = ["name", "docstatus", "items", "amended_from", "naming_series"]
#         for field in source_doc.meta.fields:
#             if not field.no_copy and field.fieldname not in exclude_fields:
#                 new_si.set(field.fieldname, source_doc.get(field.fieldname))

#         # IMPORTANT: Manually set name and clear naming_series to force our format
#         new_si.name = target_name
#         new_si.naming_series = "" 

#         # 3. Copy & Split Item Rows
#         for item in source_doc.items:
#             new_item = new_si.append("items", {})
#             exclude_item_fields = [
#                 "name", "parent", "parenttype", "parentfield", 
#                 "serial_and_batch_bundle", "serial_no", "batch_no"
#             ]

#             for f in item.meta.fields:
#                 if f.fieldname not in exclude_item_fields:
#                     new_item.set(f.fieldname, item.get(f.fieldname))

#             new_item.qty = flt(item.qty) / split_count
#             # Recalculate amounts based on new qty
#             new_item.amount = flt(new_item.qty) * flt(new_item.rate)
#             new_item.base_amount = flt(new_item.amount) * flt(source_doc.conversion_rate or 1)

#         # 4. Insert into Database
#         # ignore_links=True helps if there are strict naming validation dependencies
#         new_si.insert(ignore_permissions=True, ignore_links=False)
#         new_invoices.append(new_si.name)
...
#     # Check if container details exist and are divisible
#     container_count = len(source_doc.get("container_detail") or [])
#     if container_count % split_count != 0:
#         frappe.msgprint(f"Warning: {container_count} containers cannot be split equally into {split_count} invoices.")

#     suffixes = list(string.ascii_uppercase)
#     new_invoices = []
#     name_parts = source_doc.name.split("-")

#     # Calculate how many containers go into each new invoice
#     containers_per_invoice = int(container_count / split_count) if container_count > 0 else 0
...
#         else:
#             modified_parts[0] = f"{modified_parts[0]}{suffix}"
#         new_si.name = "-".join(modified_parts)

#         # 2. Copy Parent Fields
#         exclude_fields = ["name", "docstatus", "items", "container_detail", "amended_from", "naming_series"]
#         for field in source_doc.meta.fields:
#             if not field.no_copy and field.fieldname not in exclude_fields:
#                 new_si.set(field.fieldname, source_doc.get(field.fieldname))
...
#         # 3. Copy & Split Item Rows (Qty split)
#         for item in source_doc.items:
#             new_item = new_si.append("items", {})
#             for f in item.meta.fields:
#                 if not f.no_copy and f.fieldname not in ["name", "parent"]:
#                     new_item.set(f.fieldname, item.get(f.fieldname))
...
#         # 4. DISTRIBUTE CONTAINER DETAILS
#         # We take a slice of the original container list for each split
#         if containers_per_invoice > 0:
#             start_idx = i * containers_per_invoice
#             end_idx = start_idx + containers_per_invoice
            
#             # Extract the specific rows for this split
#             subset_containers = source_doc.get("container_detail")[start_idx:end_idx]
            
#             for cont in subset_containers:
#                 new_cont = new_si.append("container_detail", {})
#                 # Copy all fields from the container row
#                 for f in cont.meta.fields:
#                     if not f.no_copy and f.fieldname not in ["name", "parent"]:
#                         new_cont.set(f.fieldname, cont.get(f.fieldname))
...
#     # Original: E-053-25-26 -> Parts: ['E', '053', '25', '26']
#     name_parts = source_doc.name.split("-")
...
#         # --- 1. ROBUST NAMING LOGIC ---
#         suffix = suffixes[i]
#         modified_parts = list(name_parts)
        
#         if len(modified_parts) >= 2:
#             # Targets the second part (e.g., '053' -> '053A')
#             modified_parts[1] = f"{modified_parts[1]}{suffix}"
#         else:
...
#         # Explicitly set name and clear series
#         new_si.name = target_name
#         new_si.naming_series = "" 
...
#         # --- 5. INSERT WITH FORCED NAME ---
#         # Using flags to ensure the manual name is respected
...
#     # Get container data from source
#     source_containers = source_doc.get("container_detail") or []
...
#     # Pre-fetch metadata for the target DocType to avoid errors
...
#         else:
#             modified_parts[0] = f"{modified_parts[0]}{suffix}"
#         new_split_doc.name = "-".join(modified_parts)

#         # --- COPY PARENT FIELDS ---
#         exclude_fields = ["name", "docstatus", "items", "container_detail", "naming_series", "amended_from"]
#         for field in source_doc.meta.fields:
#             if target_meta.has_field(field.fieldname) and not field.no_copy and field.fieldname not in exclude_fields:
#                 new_split_doc.set(field.fieldname, source_doc.get(field.fieldname))

#         # --- 1. COPY ITEMS (Ensure 'items' field exists in target) ---
#         if target_meta.has_field("items"):
#             for item in source_doc.items:
...
#                 new_item.amount = flt(new_item.qty) * flt(new_item.rate)
#         else:
#             frappe.log_error("Field 'items' missing in Split Sales Invoice", "Split Logic Error")

#         # --- 2. DISTRIBUTE CONTAINERS (The error was here) ---
#         if containers_per_invoice > 0 and target_meta.has_field("container_detail"):
#             start_idx = i * containers_per_invoice
#             end_idx = start_idx + containers_per_invoice
#             subset_containers = source_containers[start_idx:end_idx]
            
#             for cont in subset_containers:
#                 # This append will only fire if target_meta.has_field("container_detail") is True
#                 new_cont = new_split_doc.append("container_detail", {})
#                 for f in cont.meta.fields:
#                     if not f.no_copy and f.fieldname not in ["name", "parent"]:
#                         new_cont.set(f.fieldname, cont.get(f.fieldname))
#         elif containers_per_invoice > 0:
#              # This helps you debug if the field is named differently in your new DocType
#              frappe.msgprint(f"Warning: Record {i+1} has containers, but 'container_detail' table is missing in target DocType.")

#         new_split_doc.flags.ignore_permissions = True
#         new_split_doc.insert()
...
#         # --- 2. COPY PARENT FIELDS & DIVIDE TOTALS ---
#         exclude_fields = ["name", "docstatus", "items", "container_detail", "naming_series", "amended_from"]
        
#         # Fields that need to be divided by split_count
#         fields_to_divide = ["total_qty", "total", "base_total", "grand_total", "base_grand_total", "total_net_weight"]

#         for field in source_doc.meta.fields:
#             fname = field.fieldname
#             if target_meta.has_field(fname) and not field.no_copy and fname not in exclude_fields:
#                 val = source_doc.get(fname)
                
#                 if fname in fields_to_divide and val:
#                     new_split_doc.set(fname, flt(val) / split_count)
#                 else:
...
#                 # Divide Item Quantities and Amounts
#                 new_item.qty = flt(item.qty) / split_count
#                 new_item.amount = flt(new_item.qty) * flt(new_item.rate)
#                 new_item.base_amount = flt(new_item.amount) * flt(source_doc.conversion_rate or 1)

#         # --- 4. DISTRIBUTE CONTAINERS (EXIM TAB) ---
#         if containers_per_invoice > 0 and target_meta.has_field("container_detail"):
#             start_idx = i * containers_per_invoice
#             end_idx = start_idx + containers_per_invoice
#             subset_containers = source_containers[start_idx:end_idx]
...
#     # Metadata for the target DocType
#     target_meta = frappe.get_meta("Split Sales Invoice")
    
#     # Pre-fetch source child tables
#     source_containers = source_doc.get("container_detail") or []
#     source_quality = source_doc.get("quality_and_specification") or [] # Verify this fieldname
...
#         # --- 2. COPY PARENT FIELDS & DIVIDE TOTALS ---
#         exclude_fields = ["name", "docstatus", "items", "container_detail", 
#                           "quality_and_specification", "naming_series", "amended_from"]
        
#         # Added net_total and base_net_total here
#         fields_to_divide = [
#             "total_qty", "total", "base_total", "grand_total", 
#             "base_grand_total", "total_net_weight", "net_total", "base_net_total"
#         ]
...
#                 new_item.amount = flt(new_item.qty) * flt(new_item.rate)
#                 new_item.base_amount = flt(new_item.amount) * flt(source_doc.conversion_rate or 1)
#                 # Split item-level net amounts if they exist
#                 if hasattr(new_item, 'net_amount'):
#                     new_item.net_amount = flt(item.net_amount) / split_count

#         # --- 4. DISTRIBUTE CONTAINERS (EXIM) ---
#         if containers_per_invoice > 0 and target_meta.has_field("container_detail"):
#             start_idx = i * containers_per_invoice
#             end_idx = start_idx + containers_per_invoice
#             subset_containers = source_containers[start_idx:end_idx]
...
#             for cont in subset_containers:
#                 new_cont = new_split_doc.append("container_detail", {})
#                 # Copy every field from the container row
#                 for f in cont.meta.fields:
#                     if not f.no_copy and f.fieldname not in ["name", "parent"]:
#                         new_cont.set(f.fieldname, cont.get(f.fieldname))

#         # --- 5. COPY QUALITY AND SPECIFICATION ---
#         # Note: We usually copy the WHOLE quality table to every split invoice
#         if target_meta.has_field("quality_and_specification"):
#             for q_row in source_quality:
#                 new_q = new_split_doc.append("quality_and_specification", {})
#                 for f in q_row.meta.fields:
#                     if not f.no_copy and f.fieldname not in ["name", "parent"]:
#                         new_q.set(f.fieldname, q_row.get(f.fieldname))
...
#         suffix = suffixes[i]
#         modified_parts = list(name_parts)
#         modified_parts[1] = f"{modified_parts[1]}{suffix}" if len(modified_parts) >= 2 else f"{modified_parts[0]}{suffix}"
#         new_split_doc.name = "-".join(modified_parts)

#         # --- COPY PARENT FIELDS & SPLIT TOTALS ---
#         exclude_fields = ["name", "docstatus", "items", "amended_from"]
#         fields_to_divide = ["custom_agent","total_qty", "total", "base_total", "grand_total", "base_grand_total", "net_total", "base_net_total","base_in_words",
#                            "in_words","outstanding_amount" ]

#         for field in source_doc.meta.fields:
#             fname = field.fieldname
#             if target_meta.has_field(fname) and fname not in exclude_fields:
#                 val = source_doc.get(fname)
...
# import frappe
# from frappe.utils import flt
# import string  # <--- This import was missing or misplaced
...
#         suffix = suffixes[i]
#         modified_parts = list(name_parts)
#         # Safely handle naming parts for E-053-25-26
#         if len(modified_parts) >= 2:
#             modified_parts[1] = f"{modified_parts[1]}{suffix}"
#         else:
#             modified_parts[0] = f"{modified_parts[0]}{suffix}"
...
#         # --- COPY PARENT FIELDS & SPLIT TOTALS ---
#         exclude_fields = ["name", "docstatus", "items","amended_from", "sales_invoice_reference"]
#         fields_to_divide = [
#             "total_qty", "total", "base_total", "grand_total", 
...
#             if target_meta.has_field(fname) and fname not in exclude_fields:
#                 val = source_doc.get(fname)
#                 # Apply division for numeric fields, direct copy for others
#                 new_split_doc.set(fname, flt(val) / split_count if fname in fields_to_divide else val)
...
#             # Split the quantity and recalculate amount
#             new_item.qty = flt(item.qty) / split_count
#             new_item.amount = flt(new_item.qty) * flt(new_item.rate)
...
#     # Example: 4 containers / 2 splits = 2 containers per split
#     container_rows = source_doc.get("container_detail") or []
#     rows_per_split = len(container_rows) // split_count
...
#         # --- COPY PARENT FIELDS & SPLIT TOTALS ---
#         fields_to_divide = ["total_qty", "total", "grand_total", "net_total", "outstanding_amount","base_total","base_net_total","total_net_weight","base_grand_total"]
#         exclude_fields = ["name", "docstatus", "items", "container_detail", "amended_from"]
...
#             new_item.name = None # Clear name to let Frappe generate a new one
#             new_item.qty = flt(item.qty) / split_count
#             new_item.amount = flt(new_item.qty) * flt(new_item.rate)

#         # --- DISTRIBUTE CONTAINER DETAILS ---
#         # Get the specific slice of rows for this split
#         start_idx = i * rows_per_split
#         end_idx = start_idx + rows_per_split
        
#         # If it's the last split, take all remaining rows to handle odd numbers
#         if i == split_count - 1:
...
#             new_row.name = None # Clear child name
