...
frappe.db.get_single_value('Exim Settings', 'use_advance_authorization_license_based_on_cas_no_of_item').then(function(data) {
    if (data) {
        cur_frm.fields_dict.items.grid.get_field("advance_authorisation_license").get_query = function (doc, cdt, cdn) {
            ...
            return {
                filters: { "cas_number": d.cas_number }
            };
        };
    } else {
        cur_frm.fields_dict.items.grid.get_field("advance_authorisation_license").get_query = function (doc, cdt, cdn) {
...
frappe.ui.form.on("Delivery Note", {
    ...
    manually_enter_fob_value: function(frm){ frm.trigger("run_all_calculation"); },
    freight: function(frm){ frm.trigger("run_all_calculation"); },
    freight_calculated: function(frm){ frm.trigger("run_all_calculation"); },
    insurance: function(frm){ frm.trigger("run_all_calculation"); },
    shipping_terms: function(frm){ frm.trigger("run_all_calculation"); },
    insurance_percentage: function(frm){ frm.trigger("run_all_calculation"); },
    ...
    // 🔵 AUTO-SET COUNTRY ON PORT CHANGE
    port_of_discharge(frm) {
        if (frm.doc.port_of_discharge) {
            frappe.db.get_value("Port Details", frm.doc.port_of_discharge, "country")
                .then(r => {
...
frappe.ui.form.on("Delivery Note Item", {
    ...
    freight: function (frm) { frm.events.run_all_calculation(frm); },
    insurance: function (frm) { frm.events.run_all_calculation(frm); },
    duty_drawback_rate: function (frm) { frm.events.run_all_calculation(frm); },
    meis_rate: function (frm) { frm.events.run_all_calculation(frm); },
